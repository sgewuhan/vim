...
public class FuelLabelView extends GenericPrintabelView {
	...
	public void doCancel() {
		...
		try {
			...
			UIUtils.showMessage(getSite().getShell(), "燃油标识撤消", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
	}
	...
	public void doReUpload() {
		...
		try {
			...
			DBObject setting = VimUtils.saveUploadData(idList, memo,
					IVIMFields.COL_FUELABEL,
			...
			UIUtils.showMessage(getSite().getShell(), "燃油标识补传", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
	...
	public void doReAssembly() {
		...
		while (iter.hasNext()) {
			...
			new DataAssembly(vin) {
				...
				@Override
				protected void asyncAssemblyDone(IStatus result,
						SQLRow mesRawData, DBObject productCodeData,
						DBObject cocData, DBObject confData,
						DBObject dpcocData, DBObject dpconfData) {
					try {
						DataObjectEditorInput input = VimUtils.getFLInput(
								cocData, confData, productCodeData, mesRawData,
								null, vin, _id);
						input.save();
	...
	public void doEdit() {
		...
		IEditorSaveHandler saveHandler = new IEditorSaveHandler() {
			...
			public boolean doSaveBefore(DataObjectEditorInput input,
				...
				input.getData().getData()
						.removeField(IVIMFields.mVeh_A_update_memo);
			...
			@Override
			public boolean doSaveAfter(DataObjectEditorInput input,
					IProgressMonitor monitor, String operation)
					throws Exception {
		...
		try {
			editor = UIUtils.open((ObjectId) data.get("_id"),
					"com.sg.vim.print.editor.fuellabel_edit", true, false,
					saveHandler);
		} catch (Exception e) {
			UIUtils.showMessage(getSite().getShell(), "更新燃油标识数据",
	...
	public void updateStatus() {
		...
		String vin = (String) dbo.get(IVIMFields.F_0_6b);
		String clxh = (String) dbo.get(IVIMFields.F_0_2C1);
		String clzl = (String) dbo.get(IVIMFields.F_0_4);
		String rllx = (String) dbo.get(IVIMFields.F_25);
		String start = "2012-01-01 00:00:00";
		String end = new SimpleDateFormat(Utils.SDF_DATETIME)
				.format(new Date());
		String timetype = "MANUFACTURE_TIME";
		ArrayOfVehicleBasicInfo fuelList = VimUtils.getFUELDATAService()
				.queryUploadedFuelData(VimUtils.FUELLABEL_USERNAME,
						VimUtils.FUELLABEL_PASSWORD, 0, 100, vin, clxh, clzl,
						rllx, start, end, timetype, VimUtils.FUELLABEL_OKEY);
