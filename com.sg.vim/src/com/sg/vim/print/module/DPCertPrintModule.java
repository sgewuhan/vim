...
public class DPCertPrintModule extends PrintModule {
    ...
    public String getText() {
        ...
        if (isEnable()) {
            builder.append(FileUtil.getImageURL("doc_48.png", Vim.PLUGIN_ID, "image"));
        } else {
            builder.append(FileUtil.getImageURL("doc_d_48.png", Vim.PLUGIN_ID, "image"));
        }
    ...
    public String getDisplayedPaperNumber() {
        if (paperNumber == null) {
            if (canPrintData()) {
                int currentId = VimUtils.getCurrentMaxPaperOfDPCert();
