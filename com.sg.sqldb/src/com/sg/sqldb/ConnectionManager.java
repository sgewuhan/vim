	...
	public synchronized void release() {
		...
		while (iter2.hasNext()) {
			...
			try {
				...
				Commons.LOGGER.error("无法撤销下列JDBC驱动程序的注册: "
	...
	private void createPools(Properties props) {
		...
		while (propNames.hasMoreElements()) {
			...
			if (name.endsWith(".url")) {
				...
				if (url == null) {
					Commons.LOGGER.error("没有为连接池" + poolName + "指定URL");
					continue;
				}
				...
				try {
					...
					Commons.LOGGER.error(
							"ERROR SETTING OF: MAX CONNECTION COUNT" + maxconn
									+ " .POOL NAME: " + poolName, e);
					max = 10;
				}
				DBConnectionPool pool = new DBConnectionPool(poolName, url,
						user, password, max);
				pools.put(poolName, pool);
				Commons.LOGGER.info("CONNECTION POOL:" + poolName
						+ " INIT...OK");
			}
		}
	}
	...
	private void init() {
		try {
			InputStream is = new BufferedInputStream(new FileInputStream(
					System.getProperty("user.dir") + "/conf/db.properties"));
	...
	private void loadDrivers(Properties props) {
		...
		while (st.hasMoreElements()) {
			...
			try {
				...
				Commons.LOGGER.error(" 无法注册JDBC驱动程序: " + driverClassName, e);
			}
		}
	}
	...
	class DBConnectionPool {
		...
		public synchronized void release() {
			...
			while (allConnections.hasNext()) {
				...
				try {
					...
					Commons.LOGGER.error("无法关闭连接池" + name + "中的连接", e);
				}
			}
		...
		private Connection newConnection() {
			...
			try {
				...
				Commons.LOGGER.error("无法创建下列URL的连接: " + URL, e);
				return null;
			}
			return con;
		}
