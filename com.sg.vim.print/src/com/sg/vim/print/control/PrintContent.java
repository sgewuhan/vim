...
public class PrintContent extends Composite {
		...
		public Object function(Object[] arguments) {
			...
			if (arguments != null) {
				...
				if (module != null) {
					...
					module.setCallbackProperties("Veh_Veh_Dywym",
							mVeh_Veh_Dywym);
				}
			}
			return null;
	...
	public PrintContent(ManagedForm mform, Composite parent, int style) {
		...
		printCertResultFunction = new PrintCertResultFunction(certBrowser,
				"printCertResult");
	...
	protected void doQueryButtonPressed() {
		...
		Job job = new Job("组装合格证数据VIN" + vin) {
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				Object productCode = mesRawData
						.getValue(VimUtils.FIELD_PRODUCT_CODE);
				...
				try {
					productCodeData = VimUtils
							.getProductCodeInfo((String) productCode);
				} catch (Exception e) {
					return new Status(Status.ERROR, PrintActivator.PLUGIN_ID,
							message, e);
				}
				...
				// 判断有无底盘ID
				/**
				 * 输入VIN时，有底盘型号和底盘ID不为空，而先打印底盘合格证。如果系统中有这台车的底盘合格证就不用打印底盘合格证，
				 * 就直接打印整车合格证， 如果已存在此车的整车合格证就不用打印，。打印了底盘合格证后，再打印整车合格证时，在整车合格证位置，
				 * 打印出整车合格证编号和底盘合格证编号。
				 */
				...
				if ((dpId instanceof String) && (dpId.toString().length() > 0)) {
					...
					try {
						...
						return new Status(Status.ERROR,
								PrintActivator.PLUGIN_ID, message, e);
					}
	...
	protected PrintModule getModulebyName(PrintModule[] modules,
			String className) {
		if (modules == null) {
			return null;
		}
	...
	public void showData(PrintModule printModule) {
		...
		if (input != null) {
			MultipageEditablePanel folder = new MultipageEditablePanel(
					editorArea, SWT.TOP | SWT.FLAT);
