...
public class PrintContent extends Composite {
	...
	private Browser createCertBrowser(PrintContent printContent) {
		...
		printCertResultFunction = new BrowserFunction(certBrowser,
				"printCertResult") {
	...
	private Composite createBanner(PrintContent printContent) {
		...
		Color[] gradientColors = new Color[] {
				...
				new Color(getDisplay(), 245, 245, 250),
				new Color(getDisplay(), 220, 220, 240), };
	...
	private SashForm createContent(PrintContent printContent) {
		...
		navigator = new TreeViewer(navigatorPanel, SWT.BORDER
		...
		dataPreview
				.setMinSize(editorArea.computeSize(SWT.DEFAULT, SWT.DEFAULT));
		return sash;
	}
	...
	private void queryButtonPressed() {
		...
		if (!valid) {
			UIUtils.showMessage(getShell(), "输入VIN", "输入的VIN不合法，请重新输入。",
					SWT.ERROR);
			return;
		}
		...
		DataAssembly j = new DataAssembly(vin) {
			...
			protected void asyncAssemblyDone(IStatus result, SQLRow mesRawData,
					...
					if (result.isOK()) {
						...
						para.put(PrintModule.PARA_PRODUCT_CODE_DATA,
								productCodeData);
	...
	private PrintModule getModulebyName(PrintModule[] modules, String className) {
		...
		for (int i = 0; i < modules.length; i++) {
			if (modules[i].getName().equals(className)) {
				...
				PrintModule child = getModulebyName(modules[i].getSubModules(),
						className);
				if (child != null) {
					return child;
				}
	...
	public void showData(PrintModule printModule) {
		...
		if (input != null) {
			...
			dataPreview.setMinSize(editorArea.computeSize(SWT.DEFAULT,
					SWT.DEFAULT));
		}
	...
	public void doPrint(COCPrintModule cocPrintModule) {
		try {
			...
			if (!IVIMFields.LC_PRINTED.equals(certData
					.get(IVIMFields.LIFECYCLE))
					&& !IVIMFields.LC_UPLOADED.equals(certData
							.get(IVIMFields.LIFECYCLE))) {
			...
			VimUtils.printCOC(cocBrowser, cocPrintModule.getInput().getData()
					.getData());
		} catch (Exception e) {
			UIUtils.showMessage(getShell(), "打印",
	...
	private void setPrinter(PrintModule module, String printfunctionName)
			throws Exception {
	...
	private void doBarResultCallback(Object[] arguments) {
		if (arguments != null) {
			...
			if (mVeh_Clztxx != null
					&& "dp".equalsIgnoreCase(mVeh_Clztxx.toString())) {
				currentModule = dpCertPrintModule;
			} else if (mVeh_Clztxx != null
		...
		if (arguments != null) {
			...
			if (!Utils.isNullOrEmptyString(mVeh_ErrorInfo)) {
				...
				if (currentModule != null) {
					...
					currentModule.setValue(IVIMFields.mVeh_Dywym,
							mVeh_Veh_Dywym);
				}
				if (currentModule == dpCertPrintModule) {
					qxCertPrintModule.setValue(IVIMFields.mVeh_Dphgzbh,
							mVeh__Wzghzbh);// 将底盘生成的合格证号写入到整车数据中
				}
	...
	private void savePrintData(PrintModule module) {
		...
		actionList.add(new BasicDBObject()
				...
				.append(IVIMFields.ACTION_REC_MEMO,
						"No.:" + moduleData.get(IVIMFields.mVeh_Zzbh)));
	...
	public void doUpload(CertPrintModule certPrintModule) {
		...
		try {
			...
			UIUtils.showMessage(getShell(), "上传",
					"合格证数据上传时发生错误\n" + e.getMessage(), SWT.ICON_ERROR);
			return;
		}
