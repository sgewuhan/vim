...
public class SyncMESCert extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		try {
			SQL_QUERY(VimUtils.MES_DB,
					(TableNavigator) HandlerUtil.getActivePart(event));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void SQL_QUERY(final String dataSource, final TableNavigator part)
			throws Exception {
		...
		Job job = new Job("同步MES中已经上传的合格证数据") {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					...
					return new Status(IStatus.ERROR, PrintActivator.PLUGIN_ID,
							Status.OK, "同步时发生错误", e);
				}
	...
	protected void run(String dataSource, IProgressMonitor monitor)
			throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				try {
					monitor.setTaskName("正在处理(" + index + "): "
							+ (String) ist.get(MES_VEH_HGZBH));
	...
	private BasicDBObject importMESItem(BasicDBObject ist) {
		// 判断是否已经存在
		...
		if (value instanceof Date) {
			...
			try {
				value = ((String) value).substring(0, 4) + "年"
						+ ((String) value).substring(5, 7) + "月"
						+ ((String) value).substring(8, 10) + "日";
				cert.put(IVIMFields.mVeh_Clzzrq, value);// Veh_Clzzrq";//车辆制造日期字符14YYYY年MM月DD日
			} catch (Exception e) {
			}
		}
		...
		if (value instanceof Date) {
			...
			try {
				...
				cert.put(IVIMFields.mVeh_Ggsxrq, value);
			} catch (Exception e) {
			}
		}
		...
		cert.put("MES_" + MES_CLIENT_HARDWARE_INFO,
				ist.get(MES_CLIENT_HARDWARE_INFO));
