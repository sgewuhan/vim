...
public class CertificateView extends TableNavigator {
	...
	private void createBrowser(final Composite parent) {
		...
		printCertResultFunction = new BrowserFunction(browser,
				"printCertResult") {
	...
	private String getMemo(String title) {
		...
		InputDialog d = new InputDialog(getSite().getShell(), title, "请输入原因",
				"", validator);
	...
	private void doPrintCertResultCallback(Object[] arguments) {
		if (arguments != null) {
			...
			if ("-1".equals(jsReturn.toString())
				...
				UIUtils.showMessage(getSite().getShell(), "合格证补打",
						"您选中的合格证补打发生错误：\n" + mVeh_ErrorInfo, SWT.ICON_ERROR
								| SWT.OK);
			} else {
	...
	private void doRePrint(DBObject data) {
		...
		String lc = (String) currentPrintData.get(IVIMFields.LIFECYCLE);
		if (!IVIMFields.LC_PRINTED.equals(lc)
				&& !IVIMFields.LC_UPLOADED.equals(lc)) {
	...
	public void doReUpload() {
		...
		try {
			...
			UIUtils.showMessage(getSite().getShell(), "合格证补传", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
	}
	...
	public void doRePrint() {
		...
		InputDialog input = new InputDialog(getSite().getShell(), "合格证补打",
				"请输入新的合格证纸张编号:", initialValue, validator);
		int open = input.open();
		...
		if (maxNumber > startNumber) {
			UIUtils.showMessage(getSite().getShell(), "合格证补打",
					"输入的纸张编号已被占用\n请重新补打并设置正确的纸张编号。", SWT.ICON_ERROR | SWT.OK);
			return;
		}
	...
	public void doUpload() {
		...
		try {
			...
			UIUtils.showMessage(getSite().getShell(), "合格证上传", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
	...
	public void doCancel() {
		...
		try {
			...
			UIUtils.showMessage(getSite().getShell(), "合格证撤消", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
	}
	...
	public void doEdit() {
		...
		try {
			editor = UIUtils.open((ObjectId) data.get("_id"),
					"com.sg.vim.print.editor.certificate_edit", true, false,
					saveHandler);
		} catch (Exception e) {
			UIUtils.showMessage(getSite().getShell(), "合格证修改", e.getMessage(),
					SWT.ICON_ERROR | SWT.OK);
		}
